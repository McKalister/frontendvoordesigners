...
function toggleMovieWatchList() {
	// Get the movie name form the current open movie element
	var movieName = document.getElementById('watchListAction').dataset.movieName;
	...
	// Now rerender the movie list and the current movie
...
function renderMovieList() {
	...
	// Render the watchlist count on the counter
	document.getElementById('watchlist-counter').innerHTML = watchList.length;
...
function addItemToDOM(list, itemId, movie, watchList) {
	...
	var item = `<li class="movie" onclick="loadMovie(${itemId})">
		<div class="banner" style="background-image: url(${movie.image})"></div>
		<div class="meta">
			<h2>${movie.name}</h2>
			<ul>
				<li>${movie.time}<li>
...
function loadMovie(itemId) {
	// Load all movies & watchlist
	var movies = loadMovies();	
	var watchList = loadWatchList();

	// Reset the current state
	document.getElementById('watchListAction').classList.remove('on-watchlist');
	...
	// Check if the movie is on the watchlist
	var movieOnWatchList = watchList.indexOf(movies[itemId].name) >= 0
	// If the movie is on my watchlist add it to that list as well
	if (movieOnWatchList) {
		document.getElementById('watchListAction').classList.add('on-watchlist');
	}
	// This one is important as it sets the id of the movie on the watchlistbutton
	document.getElementById('watchListAction').dataset.movieName = movies[itemId].name;
}
 ...
 */
function nextMovie() {
	// Check if it is even possible to load a next movie, we check on the length 
	// -1 because we start at 0 with displaying movies. 
	if (currentMovieId === loadMovies().length -1) return;
	currentMovieId += 1;	
	loadMovie(currentMovieId);	
}
 ...
 */
function prevMovie() {
	// Check if it is even possible to load a previous movie by checking if we're at zero
