...
/**
 * Generate the overlay HTML and add it to the body. The data is 
 * loaded based on the movieId that is provided. This id should be 
 * an index in the movies list that is returned when loadMovies() 
 * is called. 
 * 
 * @param movieId 
 */
function showOverlay(movieId) {
	var movie = loadMovies()[movieId];
	var overlay = `
      ...
      <div class="movieDetail">
        ...
        <div class="details">
		  ...
		  <div class="description">
			...
			<ul>
				<li><b>Category:</b> ${movie.category}</li>
				<li><b>Rate:</b> ${movie.rate}</li>
				<li><b>Time:</b> ${movie.time}</li>
				</ul>
			</div>
        </div>
      </div>
	</div>`;
	document.querySelector('body').innerHTML += overlay;
}
...
function renderMovieList() {
	...
	var allMoviesList = document.getElementById('allMovies');
	...
	for (var i in movies) {
		...
		// Always add the movies to the movie list
		addItemToDOM(allMoviesList, i, movies[i], movieOnWatchList);
